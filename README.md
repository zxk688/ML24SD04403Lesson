...
（3）最多四人一组，按小组提交代码和说明文档。代码可以直接运行，如用机器学习模型，应把训练和测试代码都保留，如用深度学习，还要把模型权重保存。如进行了调参，只需保留最优参数及对应的结果即可。
（4）提交的文档内容包括：小组成员及学号，使用的python库，采用的模型和测试精度，精度计算采用sklearn库的balanced_accuracy_score(y_true, y_pred)函数
